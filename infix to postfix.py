AIM:develop a program to conbert an infix expression to postfix using stack
import array as arr
class Stack:
    def __init__(self):
        self.MAXSIZE=100
    ...
    def infix_to_postfix(self,infix):
        for i in range(len(infix)):
            if self.isoperand(infix[i]) or infix[i].isdigit():
                ...
                if self.top!=-1: self.pop()
            else:
                while self.top!=-1 and self.precedence(infix[i])<=self.precedence(self.stack[self.top]):
                    self.postfix+=self.pop()
                self.push(infix[i])
        while self.top!=-1 and self.peek()!='(':
            self.postfix+=self.pop()
...
OUTPUT:
Enter infix expression : ((a+(b^c)-d))*e(e-(a/c)))
Postfix expression is abc^+d-eeac/-*
